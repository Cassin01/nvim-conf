...
local function objs_setup(fuzzy_obj, which_obj, output_obj, choices_obj, callback)
    ...
    local black_dict = which_insert_map(which_obj.buf, { toggle })
...
local sign_place_which = function(choices_obj, which_obj, fuzzy_obj, output_obj, obj_handlers)
    ...
    for lnum, match in ipairs(matches_obj) do
        if match_front(match.key, which_line) then
            ...
            local sub = string.sub(match.key, 1 + #which_line, prefix_size)
...
local function which_setup(which_obj, fuzzy_obj, output_obj, choices_obj, callback, obj_handlers)
    ...
    au(_g, "WinEnter", function()
        vim.api.nvim_set_hl(0, "FzwWhich", { fg = fg_which, bold = true, underdashed = true })
...
local function inputlist(choices, callback, opts)
    local _opts = { prompt = "> ", selector = "which" }
    opts = opts or _opts
    ...
    local obj_handlers = objs_setup(fuzzy_obj, which_obj, output_obj, choices_obj, callback)
